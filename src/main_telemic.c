...
#define LC9                { 868300000, { ( ( DR_6 << 4 ) | DR_6 ) }, 1 }
...
static void PrepareTxFrame( uint8_t port )
{
    ...
    case 3:
        {
            uint32_t WaLev = Water_Level();
            uint16_t vdd = BoardMeasureVdd();
            ...
            AppData[0] = 0xAB;
            AppData[1] = (WaLev >> 16 ) & 0xFF;
            AppData[2] = (WaLev >> 8 ) & 0xFF;
            AppData[3] = WaLev & 0xFF;
            AppData[4] = ( vdd >> 8 ) & 0xFF;
            AppData[5] = vdd & 0xFF;
        }
        break;
    case 224:
...
static void OnDownlinkTimerEvent( void )
{
...
int main( void )
{
    ...
    //TODO: commented the next line to enable over the air activation
    //BoardGetUniqueId( DevEui ); //TODO This over writes the device EUI. Commenting this will
    //allow us to make OTA working with the DevEui we choose on the server
    //if we leave it uncommented then we have to use the DevEui generated here.
    ...
    TimerInit( &DownlinkTimer, OnDownlinkTimerEvent );
    TimerSetValue( &DownlinkTimer, 25 );
