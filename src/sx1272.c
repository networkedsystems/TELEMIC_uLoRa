 ...
 */
// void SX1272Reset( void ); commented 20/02
...
{
    // Set RESET pin to 1
    GpioMcuInit( &SX1272.Reset, RADIO_RESET, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );

    // Wait 1 ms
    DelayMs( 1 );

    // Configure RESET as input
    GpioMcuInit( &SX1272.Reset, RADIO_RESET, PIN_INPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
...
void SX1272SetOpMode( uint8_t opMode )
{
    ...
    else
    {
        ...
        /* UN required outputs (LEDS) in our board
...
bool RadioDio1Workaround( void ){
	if(SX1272.Settings.State == RF_RX_RUNNING){
		uint8_t irqs = SX1272Read( REG_LR_IRQFLAGS );
		if(irqs & (1<<7)){
			/* Rx timeout */
			SX1272OnDio1Irq();
		}else{
