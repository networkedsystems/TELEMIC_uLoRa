...
void __attribute__ ((noinline)) eepromCounters(bool save){
	if(save){
		/* Write to eeprom */
		HAL_FLASHEx_DATAEEPROM_Unlock();
		HAL_FLASHEx_DATAEEPROM_Erase(DATA_EEPROM_BASE);
		HAL_FLASHEx_DATAEEPROM_Erase(DATA_EEPROM_BASE + 4);
		HAL_FLASHEx_DATAEEPROM_Erase(DATA_EEPROM_BASE + 8);
		HAL_FLASHEx_DATAEEPROM_Erase(DATA_EEPROM_BASE + 12);
		HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_WORD, DATA_EEPROM_BASE, UpLinkCounter);
		HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_WORD, DATA_EEPROM_BASE + 4, ~UpLinkCounter);
		HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_WORD, DATA_EEPROM_BASE + 8, DownLinkCounter);
		HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_WORD, DATA_EEPROM_BASE + 12, ~DownLinkCounter);
		HAL_FLASHEx_DATAEEPROM_Lock();
	}else{
		/* Read from eeprom */
		volatile uint32_t* eeprom = (volatile uint32_t*)DATA_EEPROM_BASE;
		if(eeprom [0] == ~eeprom[1] && eeprom [2] == ~eeprom[3]){
			UpLinkCounter = eeprom [0];
		}
	}
	asm("");
}
...
static void OnRxWindow1TimerEvent( void )
{	//TODO: All symbTimeout has been changed for safety margin in the receive window.
    uint16_t symbTimeout = 6; // DR_2, DR_1, DR_0// Bertold changed this
    int8_t datarate = 0;
...
static void OnRxWindow2TimerEvent( void )
{
	...
	//TODO: The values of symbTimeout has been increase for safe margin
    uint16_t symbTimeout = 6; // DR_2, DR_1, DR_0
    uint32_t bandwidth = 0; // LoRa 125 kHz
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    //TODO: This has been changed for more speed
    //LoRaMacParamsDefaults.ChannelsDatarate = LORAMAC_DEFAULT_DATARATE;
    LoRaMacParamsDefaults.ChannelsDatarate = LORAMAC_INITIAL_DATARATE; //TODO: Start with spreading factor 7
