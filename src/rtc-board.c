 ...
 * RTC Time base in ms
 */
double calibratedRTC_ALARM_TICK_DURATION = 0.5;
double calibratedRTC_ALARM_TICK_PER_MS = 2;

#define RTC_ALARM_TICK_DURATION                     calibratedRTC_ALARM_TICK_DURATION      // 1 tick every 488us
#define RTC_ALARM_TICK_PER_MS                       calibratedRTC_ALARM_TICK_PER_MS          // 1/2.048 = tick duration in ms
...
uint16_t prevTicks;
uint32_t globalTickSum;
uint32_t globalTickCounted;
volatile bool rtcCalibrationCompleted = false;
void TIM21_interruptHandle(void){
	uint16_t sr = TIM21->SR;
	if(sr & (1<<9)){ /* overcapture */
		firstCompare = true;
		TIM21->SR = 0;
	}else if(sr & (1<<1)){ /* Capture */
		uint16_t ticks = TIM21->CCR1;
		if(firstCompare){
			firstCompare = false;
		}else{
			uint16_t increment = ticks - prevTicks;
			globalTickSum += increment;
			globalTickCounted++;
			if(globalTickCounted >= 1000){
				rtcCalibrationCompleted = true;
				/* Stop timer */
				TIM21->CCER = 0;
				TIM21->DIER = 0;
				TIM21->CR1 = 0;
...
void RTCCalibration(void)
{
  ...
  GpioMcuInit( &SX1272.DIO5, RADIO_DIO_5, PIN_ALTERNATE_FCT, PIN_PUSH_PULL, PIN_NO_PULL, GPIO_AF5_TIM21 );

  uint8_t currentOpmode = SX1272Read(REG_OPMODE);
  uint8_t currentDio = SX1272Read (REG_DIOMAPPING2);

retry:

  SX1272Write(REG_OPMODE, RF_OPMODE_SLEEP);
  SX1272Write(REG_OPMODE, RF_OPMODE_SLEEP);
  SX1272Write(REG_OPMODE, RF_OPMODE_STANDBY);
  SX1272Write(REG_OSC, 0x3);
  SX1272Write(REG_DIOMAPPING2, 0x1 << 6);

  TIM21->ARR = 0xFFFF; /* Max reload value */
  TIM21->CNT = 0; /* Set CNT to zero */
  TIM21->SMCR = (1<<14); /* Use PA1/8 as timer clock (clock from radio) */
  TIM21->OR = 0x14;   /* Connect TI1 to LSI */
  TIM21->CCER = 0x0; /* Disable TI1 */
  TIM21->CCMR1 = (1<<0)|(1<<2)|(1<<3); /* Set channel TI1 as input compare, divide by 8 */
  TIM21->DIER = (1<<1); /* Enable TI1 interrupt */
  TIM21->SR = 0;

  firstCompare = true;
  globalTickSum = 0;
  globalTickCounted = 0;
  rtcCalibrationCompleted = false;

  TIM21->CR1 = 1; /* Start counter */

  HAL_NVIC_SetPriority( TIM21_IRQn, 0, 0 );
  HAL_NVIC_EnableIRQ(TIM21_IRQn);

  TIM21->CCER = 0x1; /* Enable TI1 */

  while(!rtcCalibrationCompleted);

  SX1272Write(REG_OSC, 0x7);
  SX1272Write(REG_OPMODE, RF_OPMODE_SLEEP);
  SX1272Write(REG_DIOMAPPING2, currentDio);
  SX1272Write(REG_OPMODE, currentOpmode);

  calibratedRTC_ALARM_TICK_PER_MS =   4000000.0/(((double)globalTickSum/(double)globalTickCounted)/8.0)/19.0/1000.0;

  if(calibratedRTC_ALARM_TICK_PER_MS > 3 || calibratedRTC_ALARM_TICK_PER_MS < 1){
	  goto retry;
  }

  calibratedRTC_ALARM_TICK_DURATION = 1.0f/calibratedRTC_ALARM_TICK_PER_MS;
...
void RtcInit( void )
{
    ...
    if( RtcInitalized == false )
    {
        ...
        /* Bertold: This depends on the clock frequency of LSI and needs
           calibration (38KHz nominal frequency) */
        RtcHandle.Init.AsynchPrediv = 19 - 1;
        RtcHandle.Init.SynchPrediv = 1 - 1;
        ...
        RtcCalendarContext = RtcGetCalendar( );
...
static void RtcStartWakeUpAlarm( uint32_t timeoutValue )
{
    ...
    memset(&alarmStructure, 0, sizeof(alarmStructure));
...
static RtcCalendar_t RtcComputeTimerTimeToAlarmTick( TimerTime_t timeCounter, RtcCalendar_t now )
{
    ...
    if( timeCounter < 20 )  //TODO already fixed by adding 20 instead of 1
    {
        timeCounter = 20;
    }

    // timeoutValue is used for complete computation
    timeoutValue = round( (double)timeCounter * RTC_ALARM_TICK_PER_MS );
...
static TimerTime_t RtcConvertCalendarTickToTimerTime( RtcCalendar_t *calendar )
{
    ...
    for( uint16_t i = 0; i < ( now.CalendarDate.Year + now.CalendarCentury ); i++ )
    {
        if( ( i == 0 ) || ( i % 4 ) == 0 )
        {
...
static void RtcCheckCalendarRollOver( uint8_t year )  //TODO new21-2
{
    if( year == 99 )
    {
...
static RtcCalendar_t RtcGetCalendar( void )
{
    ...
    calendar.CalendarCentury = Century;			//TODO new21-2
